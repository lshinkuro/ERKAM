...
export const getKecamatan = async (): Promise<any> => {
  const response = await api.get<any>(
    `/v2/reference-services/kecamatan?activated=1`,
  );
  let tmp1: any = response.data.return;

  const value = JSON.stringify(tmp1);
  localStorage.setItem("kecamatan", value);
  return response.data.return;
};

export const getKecamatanbyId = async (idz: any): Promise<any> => {
  const response = await api.get<any>(
    `/v2/reference-services/kecamatan?activated=1&kode_kabkota=${idz}`,
  );
  let tmp1: any = response.data.return;

  const value = JSON.stringify(tmp1);
  localStorage.setItem("kecamatandropdown", value);
  return response.data.return;
};

export const getKelurahan = async (): Promise<any> => {
  const response = await api.get<any>(
    `/v2/reference-services/kelurahan?activated=1`,
  );
  let tmp1: any = response.data.return;
  return response.data.return;
};

export const getKelurahanbyId = async (idz: any): Promise<any> => {
  const response = await api.get<any>(
    `/v2/reference-services/kelurahan?activated=1&kode_kecamatan=${idz}`,
  );
  let tmp1: any = response.data.return;
  const value = JSON.stringify(tmp1);
  // localStorage.setItem("kelurahandropdown", value);
  return response.data.return;
};
...
export const getReferensiRencanaRincianKegiatanDropdown = async (
  tahun: string,
): Promise<any> => {
  try {
    const response = await api.get<any>(
      `/v2/reference-services/rencana-rincian-kegiatan-dropdown?tahun=${tahun}`,
    );
    const indexedDb = new IndexedDb("offline-db-action");
    await indexedDb.createObjectStore([
      "rencana-rincian-kegiatan-dropdown-offline-action",
    ]);
    const localData = await indexedDb.getAllValue(
      "rencana-rincian-kegiatan-dropdown-offline-action",
    );
