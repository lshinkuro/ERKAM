...
export const browse = async (urlParam:string): Promise<any> => {
  try {
    const response = await api.get(
      `${basePathRealization}/realizations/laporan-apbm?${urlParam}`
    )
    localStorage.setItem("laporan-apbm", JSON.stringify(response.data.return));
...
export const browseDashboard = async (urlParam:string): Promise<any> => {
  try {
    const response = await api.get(
      `${basePathRealization}/realizations/laporan-apbm-dashboard?${urlParam}`
    )
    localStorage.setItem("realizations/laporan-apbm-dashboard", JSON.stringify(response.data.return));
    return response.data.return;
  } catch(e){
    console.log("e ", e)
    return []
