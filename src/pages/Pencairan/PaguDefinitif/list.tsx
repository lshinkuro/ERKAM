...
function PaguDefinitifList() {
  ...
  const [alertType, setAlertType] =
    React.useState<AlertProps["type"]>(undefined);
  const [alertMessage, setAlertMessage] = React.useState<String>("");
  const [isReading, setIsReading] = React.useState<boolean>(false);
  const [isReadingComplete, setIsReadingComplete] =
  ...
  React.useEffect(() => {
    ...
    if (groupRole === "madrasah") {
      ...
      paguDefinitifService
        .browse({ activated: ["1"], tahun: [2021], groupRole: groupRole })
        .then((data) => {
          setDataTable(data);
  ...
  const columns = [
    ...
    },
    {
      ...
      sortable: true,
      cell: (row: any) => (
        <div className="flex items-center">{formatRupiah(row.nilai_pagu)}</div>
      ),
    },
    {
      name: "Aksi",
  ...
  async function downloadExcelTemplate() {
    ...
    try {
      const dataMadrasah: [] = JSON.parse(
        localStorage.getItem("profile-madrasah")!,
      );
      exportToXlsx(
        dataMadrasah.map((el: any) => {
          return {
            ID: el.id,
            NSM: el.nsm,
            PPK: el.kode_level_ppk,
        ...
        const elements = XLSX.utils
          ...
          .map((el: any) => {
            return {
              madrasah_id: el["ID"],
              nsm: el["NSM"],
              kode_level_ppk: el["PPK"],
              nama_madrasah: el["NAMA SEKOLAH"],
              sumber_dana: el["SUMBER DANA"],
              termin_1: el["TERMIN 1"],
              termin_2: el["TERMIN 2"],
              nilai_pagu: el["PAGU DEFINITIF"],
              no_rekening: el["NO REKENING"],
              tahun: 2021,
            };
          });
  ...
  return (
    <>
      {isAlertOpen ? <Alert type={alertType}>{alertMessage}</Alert> : null}
        ...
        <HeadTable>
          {groupRole === "pusat" ? (
            <HeadTable>
              <Button
                className="mr-2"
                disabled={isAlertOpen && alertType === "info"}
