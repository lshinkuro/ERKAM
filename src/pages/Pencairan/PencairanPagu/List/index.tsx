...
function PencairanPagu() {
  const itemBreadcrumb = [
    ...
    { path: "/", breadcrumbName: "Pencairan Pagu" },
  ...
  const [isInputModalOpen, setIsInputModalOpen] =
  ...
  React.useEffect(() => {
    const response =
      JSON.parse(localStorage.getItem("pencairan-pagu-definitif")!) || [];
  ...
  const columns = [
    ...
    // },
    {
      ...
      cell: (row: any) => (
        <div>
          <Badge>
            {row.dokumen_pencairan_pagu_definitif.length > 0
              ? "uploaded"
              : "menunggu"}
          </Badge>
        </div>
    ...
    },
    {
      ...
      cell: (row) => (
        <div>
          <Badge>
            {row.dokumen_pencairan_pagu_definitif.length > 1
              ? "uploaded"
              : "menunggu"}
          </Badge>
        </div>
    ...
    },
    {
      ...
      cell: (row) => (
        <div>
          <Badge>
            {row.dokumen_pencairan_pagu_definitif.length > 2
              ? "uploaded"
              : "menunggu"}
          </Badge>
        </div>
    ...
    },
    {
      ...
      cell: (row) => (
        <div>
          <Badge>
            {row.dokumen_pencairan_pagu_definitif.length > 3
              ? "uploaded"
              : "menunggu"}
          </Badge>
        </div>
  ...
  return (
    <>
      <BreadCrumb routes={itemBreadcrumb} title="Pencairan Pagu" />
      <ModalUpload
        isOpen={isModalOpen}
        onClose={hideDetails}
        data={modalData}
