...
import { getRealisasiPindahBukuLogs } from "../../../../services/v2/realizationservice/pindahbukuservices";
...
const LogPindahBuku = () => {
  ...
  const getData = async () => {
    setLoading(true);
    const dataTmp = await getRealisasiPindahBukuLogs();
  ...
  let columns: any = [
    ...
    },
    {
      ...
      render: (record) =>
        tmpTipeKas.length &&
        tmpTipeKas
          .filter((item: any) => item.kode === record.logs.to_kode_tipe_kas)
          .map((item) => item.nama),
    },
    {
      title: "Jumlah",
  ...
  return (
    <>
      <BreadCrumb
        ...
        title="Log Realisasi Pindah Buku"
        back={true}
        toBack={() => route.goBack()}
      />
