...
import HeaderBelanja from "./Component/HeaderBelanja";
import { uuidv4 } from "../../../../utils/helper";
import notifAlert from "../../../../components/NotifAlert";
import { setStore } from "../../../../redux/actions";
import {
  deleteRencanaKegiatanIndikatif,
  editRencanaKegiatanIndikatif,
  postRencanaKegiatanIndikatif,
} from "../../../../services/v2/planningservice/rencanakegiatanindikatif";
import ModalFormAction from "./Component/ModalFormAction";

/**
 * Halaman awal Rencana Pagu Indikatif Belanja
 */

const BelanjaIndikatif = () => {
  ...
  const tmpRencanaBelanjaIndikatif = store.rencanaKegiatanIndikatif || [];
  const tmpRencanaRekapSumberDanaBelanja =
    store.rencanaRekapSumberDanaBelanja || [];
  ...
  let dataTable: any = search
    ? tmpRencanaBelanjaIndikatif.filter((item: any) => {
      ...
      render: (record) => (
        <Space>
          <ButtonTableDetail
            ...
            onClick={() =>
              route.push({
                pathname: "/rencana/indikatif/belanja/rincian",
                state: record,
              })
            }
          />
  ...
  const handleDelete = async (record: any) => {
    const rencanaKegiatanIndikatif = tmpRencanaBelanjaIndikatif.filter(
      (item: any) => !item.id.includes(record.id),
    );
    try {
      await deleteRencanaKegiatanIndikatif(record.id);
      notifAlert({
        type: "success",
        description: "Penghapusan Data Belanja Indikatif Berhasil",
      });
      setTimeout(() => {
  ...
  const handleSave = async (values: any) => {
    setLoading(true);
    const namaSnp = tmpStandarPendidikan.find(
      (item: any) => item.kode === values.standarPendidikan,
    );
    const namaKegiatan = tmpKegiatan.find(
      (item: any) => item.kegiatan.kode === values.kegiatan,
    );
    const namaSubKegiatan = tmpSubKegiatan.find(
    ...
    const payload = {
      ...
      nama_snp: namaSnp?.nama,
      kode_kegiatan: values.kegiatan,
      nama_kegiatan: namaKegiatan?.kegiatan.nama,
      kode_sub_kegiatan: values.subKegiatan,
      nama_sub_kegiatan: namaSubKegiatan?.nama,
    ...
    console.log(payload);
    if (title === "Tambah") {
      // const rencanaKegiatanIndikatif = [...tmpRencanaBelanjaIndikatif, payload]; //offline mode
      try {
        const tmpData = await postRencanaKegiatanIndikatif(payload);
        const rencanaKegiatanIndikatif = [
          ...tmpRencanaBelanjaIndikatif,
          tmpData,
        ];
        notifAlert({
          type: "success",
          description: "Data belanja indikatif berhasil disimpan",
        });
        setTimeout(() => {
          dispatch(setStore({ rencanaKegiatanIndikatif }));
        }, 100);
      ...
      const filterRencanaBelanjaIndikatif = tmpRencanaBelanjaIndikatif.filter(
        (item: any) => !item.id.includes(id),
      );
      try {
        const tmpEdit = await editRencanaKegiatanIndikatif(payload);
        const rencanaKegiatanIndikatif = [
          ...filterRencanaBelanjaIndikatif,
          tmpEdit,
        ];
        notifAlert({
          type: "success",
  ...
  const handleExport = () => {
    setLoading(true);
    try {
      let xls = tmpRencanaBelanjaIndikatif.map((item: any) => {
        let tmpSasaran: any = "";
      ...
      ExportToExcel(xls, "daftar-belanja-indikatif");
  ...
  return (
    <>
      <BreadCrumb routes={itemBreadcrumb} title="Rencana Belanja Indikatif" />
      <HeaderBelanja data={tmpRencanaRekapSumberDanaBelanja} />
      <div className="m-5 p-5 bg-white shadow-md rounded">
        <div className="mb-4">
          <NotifMessage
            tanggalErkam={tmpRencanaTanggal}
            module="belanjaIndikatif"
          ...
          <div className="mr-4">
            <Space size={[8, 8]} wrap>
              ...
              <ButtonDetail
                title="Rincian"
                disabled={btnAction}
                onClick={() => route.push("/rencana/indikatif/belanja/rincian")}
              />
              <ButtonExport
                title="Export"
                loading={loading}
              ...
              />
              <ButtonLog
                onClick={() => route.push("/rencana/indikatif/belanja/logs")}
              />
            </Space>
          </div>
          <InputSearch
...
export default BelanjaIndikatif;
