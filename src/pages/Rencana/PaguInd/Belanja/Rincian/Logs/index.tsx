...
const RincianLogs = () => {
  ...
  const getData = async (rencanaKegiatanID) => {
    ...
    const dataTmp = await getPlanningAll(
      "rencana-rincian-kegiatan-indikatif/logs/kegiatan",
      {
        rencanaKegiatanIndikatifId: rencanaKegiatanID,
      },
    );
  ...
  let columns: any = [
    ...
    },
    {
      ...
      key: "komponen",
      render: (record) =>
        record.rencanaRincianKegiatanIndikatif.komponenBiayaNama,
    },
    {
      title: "Jenis Belanja",
      key: "akunBelanja",
      render: (record) =>
        record.rencanaRincianKegiatanIndikatif.namaJenisBelanja,
    },
    {
      title: "Pajak",
      key: "pajak",
      render: (record) => record.rencanaRincianKegiatanIndikatif.pajak,
    },
    {
      title: "Tipe Pencairan",
      key: "tipePencairan",
      render: (record) =>
        record.rencanaRincianKegiatanIndikatif.tipePencairanNama,
    },
    {
      title: "Status",
      key: "status",
      render: (record) => {
        let color: any =
          (record.rencanaRincianKegiatanIndikatif.status === "MENUNGGU" &&
            "#ffca27") ||
          (record.rencanaRincianKegiatanIndikatif.status === "DITOLAK" &&
            "#f44436") ||
          (record.rencanaRincianKegiatanIndikatif.status === "DISETUJUI" &&
            "#008000");
        return (
          <Tag color={color}>
            {" "}
            {record.rencanaRincianKegiatanIndikatif.status}
          </Tag>
        );
      },
    },
  ...
  return (
    <>
      <BreadCrumb
        ...
        title={"Log Rincian Belanja Indikatif"}
      />
